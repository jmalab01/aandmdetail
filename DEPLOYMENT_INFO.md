...
**Important:** Work from the `AandMdetails` folder. This folder now includes its own GitHub Pages workflow so changes made here are the changes that get deployed.
...
1. Commit your changes from inside the `AandMdetails` folder.
2. Push to the `main` branch of `github.com/jmalab01/aandmdetail`.
3. GitHub Actions runs `.github/workflows/deploy.yml`.
4. The workflow uploads the current folder (`.`) to GitHub Pages.
5. Changes are available at aandmdetails.com within 1-2 minutes.

**No Netlify involved!**

If you deploy the same folder to Netlify, `netlify.toml` now uses revalidation for HTML, CSS, and JS so browser caching does not hide recent updates.
